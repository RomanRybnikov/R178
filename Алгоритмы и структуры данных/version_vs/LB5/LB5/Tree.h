...
class Tree
{
	...
	{
		std::cout << "начинаем парсить поток данных" << std::endl;
		// очистка
		delete m_Root;
...
public:
	...
	void Add(T value) {
		...
		auto count = Count() + 1;
		std::cout << "всего элементов в дереве: count=" << count - 1 << std::endl;
		auto arr = new T[count];
		std::cout << "создаем массив arr размером на 1 больше: count=" << count << std::endl;

		// вставка в массив всего существующего
		int index = 0;
		std::cout << "начинаем заполнять массив существующими значениями" << std::endl;
		Node::Add(m_Root, arr, index);

		// вставка элемента
		arr[index] = value;
		std::cout << "вставка добавляемого элемента " << value << " вконец массива" << std::endl;
